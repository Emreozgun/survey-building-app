...
class TokenService {
    public generateToken(tokenData: TokenData): string {
        return jwt.sign({userId: tokenData.userId, type: tokenData.type}, envConfig.jwt.secret, {
    ...
    public async generateAuthTokens (userId: string) {
        ...
        const accessToken = this.generateToken({userId, type: tokenTypes.ACCESS, expiresIn: accessExpirationSeconds});
        ...
        const refreshToken = this.generateToken({userId, type: tokenTypes.ACCESS, expiresIn: refreshExpirationSeconds});
